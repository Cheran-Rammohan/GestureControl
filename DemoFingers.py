...
mqttBroker = "172.20.10.9"  # Connecting to broker
...
while True:
    ...
    if len(lmList) != 0:
        ...
        #   Thumb
        if lmList[tipIds[0]][1] > lmList[tipIds[0] - 1][
            1]:  # sees if the tip of the thumb is to the right or left of the point below the thumb, (right hand only)
            fingers.append(1)
        else:
            fingers.append(0)
        #   For fingers except thumb
        for id in range(1, 5):
            if lmList[tipIds[id]][2] < lmList[tipIds[id] - 2][
                2]:  # if the fingerId is lower than the fingerId -2 (open or closed)
        ...
        img[0:h, 0:w] = overLayList[totalFingers - 1][0:h,
                        0:w]  # overlays the image on top of the video recording   (My code)

        # ====================================#
        # Relay Finger Count to MQTT Subcriber#
        if delay_mqtt >=50:
            client.publish("CountD", totalFingers)
            print(f"Finger Count Has Been Sent To Home Assistant")
